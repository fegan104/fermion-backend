...
/*Meeting tests*/
	...
	public void testTimeslots() {
	    ...
	    try {
	    	...
	    	//delete mondays at 12:20, down to 32 (1 slot * 2 mondays = -2)
	    	td.delete(calendar.getId(), DayOfWeek.MONDAY, null, LocalTime.of(12, 20));
	    	assertEquals (32, ((td.getByCalendar(calendar.getId()).get().size())));

	    	//(3 more test cases for code coverage of the non-use-case close requests)
	    	//delete all tuesdays, with just dayOfWeek (5 per day * 2 tuesdays = -10)
	    	td.delete(calendar.getId(), DayOfWeek.TUESDAY, null, null);
	    	assertEquals (22, ((td.getByCalendar(calendar.getId()).get().size())));

	    	//delete Thursdays or 12/17, down to 14 (4 left on 12/17 + 4 slots on the 1 thursday = -8)
	    	td.delete(calendar.getId(), DayOfWeek.THURSDAY, LocalDate.of(2018, 12, 17), null);
	    	assertEquals (14, ((td.getByCalendar(calendar.getId()).get().size())));
	    	
	    	//delete Wednesdays, and 10/10 at 13:40, down to 3 (5 slots per wednesday * 2 wednesdays + 1 slot 10/10 13:40 = -11)
	    	td.delete(calendar.getId(), DayOfWeek.WEDNESDAY, LocalDate.of(2018, 12, 10), LocalTime.of(13, 40));
	    	assertEquals (3, ((td.getByCalendar(calendar.getId()).get().size())));
